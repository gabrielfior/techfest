...
{  
  ...
  */
  ...
  int answer_color = analyse_spin(gyro.gyro.x,gyro.gyro.y,gyro.gyro.z, accel.acceleration.x,accel.acceleration.y,accel.acceleration.z);
  if (answer_color == 0){
      ...
      colorWipe(strip.Color(0,0, 0), 1); // White 
    }
   else {
...
int analyse_spin(double wx,double wy, double wz, double ax, double ay, double az){
        double wabs = sqrt(wx*wx + wy*wy + wz*wz);
        double a = sqrt(ax*ax + ay*ay + az*az)-9.81;
        //Serial.print(wx); Serial.print("   "); Serial.print(wy); Serial.print("   "); Serial.print(wz); Serial.println("   ");
